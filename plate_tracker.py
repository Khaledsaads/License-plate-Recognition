        ...
        try:
            while ret:
                ...
                if track_results and track_results.boxes.id  is not None:
                    boxes = track_results.boxes.xyxy.cpu().tolist()
                    ids = track_results.boxes.id.cpu().tolist()
                    clss = track_results.boxes.cls.cpu().tolist()
                    scores = track_results.boxes.conf.cpu().tolist()
                    # if class in vehicles add it to IDs
                    for class_id, box, id in zip(clss, boxes, ids):
